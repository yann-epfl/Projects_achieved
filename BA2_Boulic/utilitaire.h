...
// Fonctions de dessin (Cadre, robot, particule) :
void cadre();
...
//Determine si la ligne est un commentaire
bool commentaire(char ligne[]);

//Se deplace dans la ligne jusqu'a trouver une donnee et renvoie vrai si elle en a 
//trouve une et faux dans le cas contraire
bool donnee(char ligne[], int *p_i);

//Remplit une ligne temporaire avec une donnee
void remplit_ligne_temp(char ligne[], int *p_i, char ligne_temp[]);

//Determine si il reste des donnees dans la ligne
